...
def device_info_query(table_name, device_name, fields):
    return f'''{table_name}
| where DeviceName startswith "{device_name}"
| project {fields}
| order by TimeGenerated desc'''

def device_file_events_query(table_name, device_name, fields):
    return f'''{table_name}
| where DeviceName startswith "{device_name}"
| project {fields}
| order by TimeGenerated desc'''

def device_process_events_query(table_name, device_name, fields):
    return f'''{table_name}
| where DeviceName startswith "{device_name}"
| project {fields}
| order by TimeGenerated desc'''
